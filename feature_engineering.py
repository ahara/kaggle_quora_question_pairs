...
stop_words2 = ['the','a','an','and','but','if','or','because','as','what','which','this','that','these','those','then',
...
def text_to_wordlist(text, remove_stop_words=True, stem_words=False):
    ...
    if remove_stop_words:
        text = text.split()
        text = [w for w in text if w not in stop_words2]
        text = " ".join(text)
...
def clean(df):
    ...
    df.loc[:, 'question1'] = df.loc[:, 'question1'].apply(text_to_wordlist)
    df.loc[:, 'question2'] = df.loc[:, 'question2'].apply(text_to_wordlist)
...
magic_feature_3.run_kcore(train, test)
magic_feature_3.run_kcore_max()
...
for fname in ['train', 'test']:
    ...
    for batch_id in xrange(6):
        ...
        data.to_csv('../data/%s_features_v4.csv' % fname, index=False, encoding='utf-8')
